...
void ISO::ExtractPic1(std::string pic_path)
{
	if ( mFin.is_open() )
	{
		char *sectorBuf = (char*)malloc( ISO::SECTOR_SIZE );
		...
		if ( err >= 0 )
		{
			...
			if ( (dirId = this->findDirPathTable( "PSP_GAME/" )) > 1 )
			{
				...
				DirectoryRecord* dir = (DirectoryRecord*)sectorBuf;
				DirectoryRecord* pic1 = findFile( "PIC1.PNG", dir );

				if (pic1) {
					this->write(pic1->lba.LE, pic1->fileSize.LE, pic_path);
				}
			}
...
void* ISO::ExtractSfoToMemory(uint32_t* out_size)
{
	...
	if ( mFin.is_open() )
	{
		...
		if ( err >= 0 )
		{
			...
			if ( (dirId = this->findDirPathTable( "PSP_GAME/" )) > 1 )
			{
				...
				if (sfo) {
					void* sfo_data = this->read(sfo->lba.LE, sfo->fileSize.LE);
					if (out_size && sfo_data) {
						*out_size = sfo->fileSize.LE;
					}
					free(pathTableBuf);
					mPathTable.clear();
					free(sectorBuf);
