...
void CSO::ExtractPic1(std::string pic_path)
{
	// CSO inherits from ISO, so we can use the base class implementation
	// The readSector method is overridden to handle decompression
	ISO::ExtractPic1(pic_path);
}

void* CSO::ExtractSfoToMemory(uint32_t* out_size)
{
	// CSO inherits from ISO, so we can use the base class implementation
	// The readSector method is overridden to handle decompression
